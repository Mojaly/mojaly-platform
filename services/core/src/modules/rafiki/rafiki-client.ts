...
} from './backend/generated/graphql.js'
import {
  createAssetMutation,
  getAssetQuery,
  getAssetsQuery
...
import {
  ...
  getOutgoingPaymentQuery,
  getOutgoingPayments
} from './backend/request/outgoing-payment.request.js'
import type { OutgoingPaymentsGqlResponse } from './backend/request/outgoing-payment.request.js'
import {
  createWalletAddressKeyMutation,
  revokeWalletAddressKeyMutation
} from './backend/request/wallet-address-key.request.js'
import {
  createWalletAddressMutation,
  updateWalletAddressMutation
} from './backend/request/wallet-address.request.js'
import {
  createQuoteMutation,
  getQuoteQuery
} from './backend/request/quote.request.js'
import {
  createReceiverMutation,
  getReceiverQuery
} from './backend/request/receiver.request.js'
...
export class RafikiClient implements IRafikiClient {
  ...
  public async createReceiver(params: CreateReceiverParams): Promise<Receiver> {
    const walletAddressUrl = params.walletAddressUrl
  ...
  public async withdrawLiqudity(eventId: string) {
    ...
    if (!response.withdrawEventLiquidity?.success) {
      ...
      //   return true
      // }
      throw new Error('Unable to withdraw liquidity from Rafiki')
    }
  ...
  public async listOutgoingPayments(args: QueryOutgoingPaymentsArgs) {
    ...
    return response.outgoingPayments.edges.map(
      (el) => el.node as OutgoingPaymentsGqlResponse
    )
  }
